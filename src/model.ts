...
export const setupDecoder = (tf: ITensorflow) => {
	const decoderLayers: ILayerData[] = [
		...
		tf.layers.conv2dTranspose({filters: 32, kernelSize: 3, activation: "relu", padding: "same"}), // Output: same
		tf.layers.upSampling2d({}), // Output: 152x200
		tf.layers.conv2dTranspose({filters: 3, kernelSize: 3, padding: "same"}), // Output: same
...
export function autoEncoderLoss(inputs: Tensor, outputs: Tensor[], tf: ITensorflow) {
	return tf.tidy(() => {
		const originalDim = inputs.shape[1] ?? -1 // inputs: [batches, width, height, channels]
		...
		const reconstructionLoss = tf.losses.meanSquaredError(inputs, decoderOutput).mul(originalDim * 2)
